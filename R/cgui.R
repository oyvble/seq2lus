...
#' @param type The object type of ff (type = {"file","list","table"})
#' @export
...
#source("cgui.R");
#cgui()
#envirfile = "C:\\Users\\oyvbl\\Documents\\str2lusTEST\\proj"
#dat=euroformix::tableReader("C:\\Users\\oyvbl\\Dropbox\\Forensic\\MixtureProj\\myDev\\seq2lusDev\\CS_simulated_UASSEQ\\evid\\evids2.txt")
#lookupf=NULL;type="LUS+";toFile=FALSE;outf="converted.csv";printMissing=TRUE;missf = "MissingSequences.csv"

cgui = function(envirfile=NULL) {
 require(seq2lus)
 .sep <- .Platform$file.sep 
 pgkPath <- path.package("seq2lus", quiet = FALSE) # Get package path.
 txtformat = c("RU","LUS","LUS+")
 ...
 #helpfunction to get folder/IDs vector from list (taken from list envir list only)
  ...
  openSetup = function() { 
   #vars = names(optL)
   vars = c("workdir","lookupfile","importDataFile","outputfold","outputname","missingfile","format","LUSsymb")
...
############################################################
  runConvertion = function(h,...) { #CALL CONVERTION
	...
	datFiles = unique(datFiles ) #get unique files to import
	...
	bigdat = numeric() #assume same number of columns
	 ...
	 }, error = function(e) e) #skip files giving errors when importing
     }

	if(nrow(bigdat)==0) { #handle the situation of no data
	     gWidgets::gmessage("No data to convert was found!")		
		return()
     }
	#"Missing-allele" file (can be named in GUI):
	missf =  paste0(opt$outputfold,.sep,opt$missingfile,".csv")
     tab <- convert(bigdat,lookupf=lookupfile,type=opt$format,toFile=TRUE,outf=outf,printMissing=TRUE, missf = missf,LUSsymb=opt$LUSsymb)
     #write.table(tab,file=outf,row.names=FALSE,sep="\t",quote=FALSE) # SHOULD USER SELECT SEPARATOR?

     gWidgets::gmessage("Convertion successfully completed!")
  } #end run function
 ...
 #helpfunction for adding Folder/File when clicking button
  f_addFolder = function(h,...) {
    if(h$action=="DATFOLDS") {
     ...
     gWidgets::enabled(tab1a[2,2]) = TRUE
    }

    if(h$action=="DATFILES") {
     dirfile = gWidgets::gfile(text="Select file",type="open")
     if(is.na(dirfile)) return() 
     fv = addFold(foldadd=dirfile,h$action) #Add folder to environment and file, h=list(action="EVID")
     tab1b[1,2][] = fv #update combolist
     gWidgets::enabled(tab1b[2,2]) = TRUE
    }
  }
  ...
  f_delFolder = function(h,...) {
      if(h$action=="DATFOLDS") {
        sel = gWidgets::svalue(tab1a[1,2])
        vals = tab1a[1,2][]
        folds = setdiff(vals,sel)
        tab1a[1,2][] = folds  #folds #update combolist
        if(length(folds)==0) {
          tab1a[1,2][] = longspace #folds #update combolist
          gWidgets::svalue(tab1a[1,2]) = longspace 
          gWidgets::enabled(tab1a[2,2]) = FALSE
        } else {
          gWidgets::svalue(tab1a[1,2]) = folds[1]
        }
      }
      if(h$action=="DATFILES") {
        sel = gWidgets::svalue(tab1b[1,2])
        vals = tab1b[1,2][]
        folds = setdiff(vals,sel)
        tab1b[1,2][] = folds #update combolist
        if(length(folds)==0) {
          tab1b[1,2][] = longspace #folds #update combolist
          gWidgets::svalue(tab1b[1,2]) = longspace 
          gWidgets::enabled(tab1b[2,2]) = FALSE
        } else {
          gWidgets::svalue(tab1b[1,2]) = folds[1]
        }
      }
...
##################################################################################################
 ...
 gWidgets::addHandlerUnrealize( mainwin, handler = function(h,...) {
	bool = gWidgets::gconfirm("Are you sure you want to quit?") 
     if(bool) {
       gWidgets::dispose(mainwin) #remove window!
     } else {
	  return(TRUE)
 ...
 tabconvert = gWidgets::ggroup(expand=TRUE,spacing=spc,container=nb,label="Convert") #tab1: (select project and file storage)
...
#####################################################
  ...
  tab1 <- gWidgets::glayout(spacing=spc,container=tabconvert ) 


  tab1a = gWidgets::glayout(spacing=spc,container=(tab1[1,1] <-gWidgets::gframe("Selected folders with data",container=tab1))) 
  tab1a[1,1] <- gWidgets::glabel("Selected folders:",container=tab1a)
  tab1a[2,1] <- gWidgets::gbutton("Add a folder",container=tab1a,handler=f_addFolder,action="DATFOLDS")
  tab1a[2,2] <- gWidgets::gbutton("Remove marked folder",container=tab1a,handler=f_delFolder,action="DATFOLDS")
  folds = getFolds("DATFOLDS") 
  if(is.null(folds) || length(folds)==0)  folds = longspace #numeric()
  tab1a[1,2] <- gWidgets::gcombobox(items=folds,container=tab1a)
  if(folds[1] == longspace) gWidgets::enabled(tab1a[2,2]) = FALSE
 
  tab1b = gWidgets::glayout(spacing=spc,container=(tab1[2,1] <-gWidgets::gframe("Selected data files",container=tab1))) 
  tab1b[1,1] <- gWidgets::glabel("Selected files:",container=tab1b)
  tab1b[2,1] <- gWidgets::gbutton("Add a file",container=tab1b,handler=f_addFolder,action="DATFILES")
  tab1b[2,2] <- gWidgets::gbutton("Remove marked file",container=tab1b,handler=f_delFolder,action="DATFILES")
  folds = getFolds("DATFILES") 
  if(is.null(folds) || length(folds)==0)  folds = longspace #numeric()
  tab1b[1,2] <- gWidgets::gcombobox(items=folds,container=tab1b)
  if(folds[1] == longspace) gWidgets::enabled(tab1b[2,2]) = FALSE


  tab1c = gWidgets::glayout(spacing=spc,container=(tab1[3,1] <-gWidgets::gframe("Import Data file (customizing data format)",container=tab1))) 
  tab1c[1,1] <- gWidgets::gbutton("Selected importData file:",container=tab1c,handler = 
	function(h,...) { 
      ...
      if(!is.na(fsel)) {
       ...
       gWidgets::svalue(tab1c[1,2]) = fsel
      }
  })
  tab1c[1,2] <- gWidgets::glabel(get("setup",envir=mmTK)$importDataFile,container=tab1c)
  tab1c[2,1] <- gWidgets::gbutton("Set back to default",container=tab1c,handler = 
	function(h,...) { 
       opt = get("setup",envir=mmTK) #get
       opt$importDataFile = emptyName
       assign("setup",opt,envir=mmTK) #set to envir
       saveSetup(opt) #Save to file
       gWidgets::svalue(tab1c[1,2]) = emptyName
     })


  tab1d = gWidgets::glayout(spacing=spc,container=(tab1[4,1] <-gWidgets::gframe("Lookup table file selection",container=tab1))) 
  tab1d[1,1] <- gWidgets::gbutton("Selected LookupTable file:",container=tab1d,handler = 
	function(h,...) { 
      fsel = gWidgets::gfile(text="Select file (xlsx)",type="open")
      if(!is.na(fsel)) {
       opt = get("setup",envir=mmTK) #get
       opt$lookupfile = fsel
       assign("setup",opt,envir=mmTK) #set to envir
       saveSetup(opt) #Save to file
       gWidgets::svalue(tab1d[1,2]) = fsel
      }
  })
  tab1d[1,2] <- gWidgets::glabel(get("setup",envir=mmTK)$lookupfile ,container=tab1d)
  tab1d[2,1] <- gWidgets::gbutton("Set back to default",container=tab1d,handler = 
	function(h,...) { 
       opt = get("setup",envir=mmTK) #get
       opt$lookupfile = emptyName 
       assign("setup",opt,envir=mmTK) #set to envir
       saveSetup(opt) #Save to file
       gWidgets::svalue(tab1d[1,2]) = emptyName
     })

  tab1e = gWidgets::glayout(spacing=spc,container=(tab1[5,1] <-gWidgets::gframe("Output",container=tab1))) 
  tab1e[1,1] <- gWidgets::gbutton("Selected output folder:",container=tab1e,handler = 
	function(h,...) { 
      fsel = gWidgets::gfile(text="Select folder",type="selectdir")
      if(!is.na(fsel)) {
       opt = get("setup",envir=mmTK) #get
       opt$outputfold = fsel
       assign("setup",opt,envir=mmTK) #set to envir
       saveSetup(opt) #Save to file
       gWidgets::svalue(tab1e[1,2]) = fsel
      }
  })
  tab1e[1,2] <- gWidgets::glabel(get("setup",envir=mmTK)$outputfold,container=tab1e)

  tab1e[2,1] <- gWidgets::gbutton("Selected output filename:",container=tab1e,handler = 
	function(h,...) { 
      fsel = gWidgets::ginput(message="Set name of converted file",text=get("setup",envir=mmTK)$outputname )
      if(!is.na(fsel)) {
       opt = get("setup",envir=mmTK) #get
       opt$outputname = fsel
       assign("setup",opt,envir=mmTK) #set to envir
       saveSetup(opt) #Save to file
       gWidgets::svalue(tab1e[2,2]) = fsel
      }
  })
  tab1e[2,2] <- gWidgets::glabel(get("setup",envir=mmTK)$outputname,container=tab1e)

  tab1e[3,1] <- gWidgets::gbutton("Selected missing alleles filename:",container=tab1e,handler = 
	function(h,...) { 
      fsel = gWidgets::ginput("Set name of file with missing alleles \n(not found in lookup table)")
      if(!is.na(fsel)) {
       opt = get("setup",envir=mmTK) #get
       opt$missingfile = fsel
       assign("setup",opt,envir=mmTK) #set to envir
       saveSetup(opt) #Save to file
       gWidgets::svalue(tab1e[3,2]) = fsel
      }
  })
  tab1e[3,2] <- gWidgets::glabel(get("setup",envir=mmTK)$missingfile,container=tab1e)

  tab1f = gWidgets::glayout(spacing=spc,container=(tab1[6,1] <-gWidgets::gframe("Convert",container=tab1))) 
  tab1f[1,1] <- gWidgets::gradio(items=txtformat,container=tab1f ,horizontal = TRUE, selected=which(get("setup",envir=mmTK)$format==txtformat), handler = 
	function(h,...) {
       opt = get("setup",envir=mmTK) #get
       opt$format = svalue(tab1f[1,1])
  	  assign("setup",opt,envir=mmTK) #set to envir
       saveSetup(opt) #Save to file
     }
  )
  tab1f[2,1] <- gWidgets::gbutton("PERFORM CONVERTION",container=tab1f,handler = runConvertion)
